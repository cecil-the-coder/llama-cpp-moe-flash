...
**I10b Slot Remapping COMPLETE (2026-04-03)**: 6x SPEEDUP WITH N_SLOTS TUNING.

Image `17aca27` on b8664. Qwen3 Q4_K_M (133 GB, >GTT): **10.4-11.1 t/s** with
GPU MoE via 96-slot remapping. 6x speedup from initial 32-slot baseline (3.5 t/s).
...
- GPU compute: ~90ms per token (now the dominant cost)
- ~90 GB persistent GPU buffers (feasible on 128 GB UMA)

**Regression pass:** Qwen3 Q2_K (80 GB, <=GTT): **19.2 t/s** full GPU. PASS.

**Status**: COMPLETE -- slot remapping with N_SLOTS=96 is production-ready
...
### What didn't work
1. **Slot buffer / expert ID remapping (I10b)**: Early attempts failed from gallocr corruption (ggml_set_input/output) and ne[2] breaking graph shape inference. **Eventually solved** by moving ne[2]=N_SLOTS override to persistent pool tensors outside gallocr, with LRU slot eviction, deferred IDS write, and original IDS cache. N_SLOTS tuning from 32 to 96 delivered 6x speedup. Image `17aca27`: **10.4-11.1 t/s** with 97.1% hit rate.
...
Slot remapping with `ne[2]=N_SLOTS` on persistent pool tensors outside gallocr successfully gets expert matmul back on GPU for >GTT models. N_SLOTS tuning from 32 to 96 delivered a **6x speedup** (3.5 -> 11.1 t/s on Qwen3 Q4_K_M) by raising hit rate from 74.9% to 97.1%. At N_SLOTS=96, GPU compute (~90ms) is the dominant cost, not expert copy bandwidth. The `GGML_MOE_N_SLOTS` env var makes this tunable without rebuilding. N_SLOTS=128 exceeds RADV/UMA memory limits (OOM at ~123 GB).
